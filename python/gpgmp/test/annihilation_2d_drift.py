...
def run(options, datadir):
    ...
    # parameters
    params = "{0:s}/AB\ Annihilation\ Drift.xml --cl-source {1:s} --output abannihilationdrift.h5".format(options['inchmanexampledir'], options['inchmancldir'])
...
def analyze(datadir, filename):
    ...
    if hasmatplotlib:
        ...
        ax1.imshow(nstate[0,:,:])
        ax2.imshow(nstate[1,:,:])
        plt.savefig(datadir+"annihilation_2d_drift.eps", format="eps")
